...
{
    public class ForceSyncViewModel : BaseViewModel
    {
        public ForceSyncViewModel(INavigationService navigationService) : base(navigationService)
        {
        ...
        private async Task ExecuteSyncCommand()
        {
            ...
            {
                IsSyncing = true;
                await LocalAppDatabase.SyncEntities(true, x => TablesSyncing = x, x => TablesSynced = x);
            }
           catch (Exception e)
            {
